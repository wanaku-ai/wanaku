...
mod tests {
    ...
    use wanaku_types::audit::{
        AuditCategory, AuditDecision, AuditPersistence, AuditStore, InMemoryAuditStore,
    };
    ...
    #[test]
    fn store_shutdown_flushes_pending_events() {
        let path = std::env::temp_dir().join(format!(
            "wanaku-audit-shutdown-flush-{}.json",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&path);
        ...
        {
            let store = InMemoryAuditStore::with_persistence(10, backend.clone());
