...
// Use this when the LLM returned garbage, not when you decided to reject
...
response::reject_malformed("reason");  // "can't decide" vs "decided to reject"
...
The evaluator crate has integration tests that run entirely in-process. WASM engine tests skip automatically if the action `.wasm` files aren't built.
...
- **LLM schema validation failure**: if `result_schema` is set and the LLM output doesn't match, the engine retries once with a correction prompt. If the retry also fails, the raw result is passed to the WASM guest (which can check via `verifyLlmResult()` and call `rejectMalformed()`)
...
**Fix:**
1. **Add `result_schema` to your LLM config** — the host validates the LLM output and retries once with a correction prompt if it doesn't match. This catches most malformed responses automatically:
   ```yaml
   llm:
     result_schema:
       type: object
